...
GraphView::loadGraphFromFile ( const char *file_name )
{
  ...
  this->fname = QString::fromAscii ( file_name );
  this->finfo = QFileInfo ( file_name );
  this->title = QString::fromAscii ( "untitled" );
  ...
  while ( iter.hasNext() )
    {
      ...
      if ( attr_v != NULL )
        this->title = QString::fromAscii ( attr_v );
    }
...
GraphView::reset ( )
{
  if ( this->graph != NULL )
    {
      this->loadGraphFromFile ( this->fname.toAscii() );
      this->synchronizeView ( );
    }
}
